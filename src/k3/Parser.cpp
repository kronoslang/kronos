...
#define CHECK_ARITY(N) if (n.size() != N) return ParserError(nullptr, #N " arguments are required!");
		...
		static const std::unordered_multimap<std::string, std::pair<std::string, std::string>> builtin_metadata = {
			...
			{ "rcsbuf", { "(init order sig~)", "Provides a signal memory ring buffer with 'order' elements. The 'order' parameter must be an invariant constant. The type and initial value of the elements is determined by 'init'. Returns a tuple of (buffer output index) where 'output' is the value evicted from the buffer, 'buffer' is the content of the ring buffer prior to the current activation frame, and 'index' is the position of the write head in the ring buffer. Cyclic definitions via 'sig~' are allowed." } },
			{ "tcsbuf", { "(init duration sig~)", "Provides a signal memory ring buffer with 'duration' relative to the clock rate of `sig~`. The 'duration' must be an invariant constant. The type and initial value of the elements is determined by 'init'. Returns a tuple of (buffer output index) where 'output' is the value evicted from the buffer, 'buffer' is the content of the ring buffer prior to the current activation frame, and 'index' is the position of the write head in the ring buffer. Cyclic definitions via 'sig~' are allowed." } }
		};
